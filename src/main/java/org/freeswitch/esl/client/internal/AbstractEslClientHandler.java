...
public abstract class AbstractEslClientHandler extends SimpleChannelInboundHandler<EslMessage> {
	...
	protected void handleEslMessage(ChannelHandlerContext ctx, EslMessage message) {
		...
		final String contentType = message.getContentType();
		CompletableFuture<EslMessage> cf = null;
		...
		switch (contentType) {
			...
			case Value.COMMAND_REPLY:
				log.info("Command reply received-1 [{}]--{}", message, apiCalls.size ());
				cf = apiCalls.poll();
				log.info("Command reply received-2 [{}]--{}", message, apiCalls.size ());
				cf.complete(message);
				log.info("Command reply received-3 [{}]", message);
				break;
